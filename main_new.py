...
async def lifespan(app: FastAPI):
    logger.info("=" * 60)
    logger.info("错题系统服务端启动")
    logger.info(f"  MinerU  : {settings.mineru_base_url}")
    logger.info(f"  Qwen    : {settings.qwen_model_id} (本地推理)")
...
app = FastAPI(
    title="错题系统 API",
    version="1.0.0",
    description="MinerU + Qwen2.5-VL 两阶段流水线",
    lifespan=lifespan,
)

# Flutter 本地开发允许所有来源（生产可收窄）
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
app.include_router(save.router,    prefix="/api/v1", tags=["存储"])
...
@app.get("/", tags=["系统"])
async def root():
    return {"message": "错题系统 API v1.0", "docs": "/docs"}
