// lib/screens/home_screen.dart  （替换原文件，加入草稿横幅）
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _load() async {
    ...
    final records = await DbHelper.instance.query(
        subject: _filterSubject, grade: _filterGrade, reviewStatus: _filterStatus);
    final stats = await DbHelper.instance.stats();
    final drafts = await DraftHelper.instance.pendingTasks(); // ★ 读草稿
    if (mounted) setState(() {
      _records = records;
      _stats = stats;
      _drafts = drafts;
  ...
  Future<void> _discardDraft(DraftTask draft) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('放弃草稿', style: TextStyle(color: AppColors.textPrimary)),
        content: const Text('将删除本地草稿记录，服务端图片不受影响。',
            style: TextStyle(color: AppColors.textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('取消')),
          TextButton(onPressed: () => Navigator.pop(context, true),
              child: const Text('放弃', style: TextStyle(color: AppColors.red))),
        ],
      ),
    );
    if (ok == true) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.tune_outlined),
              onPressed: _showFilterSheet, tooltip: '筛选'),
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          slivers: [
            ...
            // 筛选 chips
            if (_filterSubject != null || _filterGrade != null || _filterStatus != null)
                  ...
                  (ctx, i) => RecordCard(
                    ...
                    onTap: () async {
                      await Navigator.push(ctx, MaterialPageRoute(
                          builder: (_) => DetailScreen(record: _records[i])));
                      _load();
                    },
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          _vDivider(),
          Expanded(child: _statItem('掌握率', '${(rate * 100).toStringAsFixed(0)}%', AppColors.amber)),
        ]),
      ),
    );
  }
  ...
  Widget _vDivider() => Container(width: 1, height: 36, color: AppColors.bg3,
  ...
  Widget _buildFilterChips() => Padding(
    ...
    child: Wrap(spacing: 6, children: [
      if (_filterSubject != null)
        _FilterChip(_filterSubject!, onRemove: () { setState(() => _filterSubject = null); _load(); }),
      if (_filterGrade != null)
        _FilterChip(_filterGrade!, onRemove: () { setState(() => _filterGrade = null); _load(); }),
  ...
  Widget _buildEmpty() {
    final hasFilter = _filterSubject != null || _filterGrade != null || _filterStatus != null;
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(hasFilter ? Icons.filter_list_off : Icons.auto_stories_outlined,
          size: 48, color: AppColors.textMuted),
      const SizedBox(height: 16),
      Text(hasFilter ? '当前筛选条件下没有记录' : '还没有错题\n点击下方按钮开始添加',
          textAlign: TextAlign.center,
          style: const TextStyle(color: AppColors.textSecondary, fontSize: 14, height: 1.6)),
      if (hasFilter) ...[
        const SizedBox(height: 16),
        TextButton(onPressed: () { setState(() { _filterSubject = null; _filterGrade = null; _filterStatus = null; }); _load(); },
  ...
  void _showFilterSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => _FilterSheet(
        subject: _filterSubject, grade: _filterGrade, status: _filterStatus,
        onApply: (s, g, st) {
          setState(() { _filterSubject = s; _filterGrade = g; _filterStatus = st; });
          _load();
  ...
  Future<void> _confirmDelete(WrongAnswerRecord r) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: AppColors.bg1,
        title: const Text('删除错题', style: TextStyle(color: AppColors.textPrimary)),
    ...
    if (ok == true) { await DbHelper.instance.delete(r.id); _load(); }
  }
}
...
class _FilterSheet extends StatefulWidget {
  ...
  const _FilterSheet({this.subject, this.grade, this.status, required this.onApply});
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  void initState() {
    super.initState();
    _sub = widget.subject; _grade = widget.grade; _status = widget.status;
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('筛选', style: TextStyle(color: AppColors.textPrimary, fontSize: 16, fontWeight: FontWeight.w600)),
        const SizedBox(height: 16),
        const Text('科目', style: AppText.label),
        const SizedBox(height: 8),
        _chips(AppConst.subjectList, _sub, (v) => setState(() => _sub = _sub == v ? null : v)),
        const SizedBox(height: 14),
        const Text('年级', style: AppText.label),
        const SizedBox(height: 8),
        _chips(AppConst.gradeList, _grade, (v) => setState(() => _grade = _grade == v ? null : v)),
        const SizedBox(height: 14),
        const Text('状态', style: AppText.label),
        const SizedBox(height: 8),
        _chips(AppConst.reviewLabels.values.toList(),
            _status != null ? AppConst.reviewLabels[_status!] : null, (v) {
          final key = AppConst.reviewLabels.entries.firstWhere((e) => e.value == v).key;
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: active ? AppColors.amber : AppColors.bg3),
          ),
...
class _FilterChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
        color: AppColors.amber.withOpacity(0.12), borderRadius: BorderRadius.circular(6),
        border: Border.all(color: AppColors.amber.withOpacity(0.4))),
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Text(label, style: const TextStyle(color: AppColors.amber, fontSize: 12, fontWeight: FontWeight.w500)),
